 ...
 * stored to x1 or x2.*/
void doubleplane(const gsl_matrix *B, const gsl_vector *t, DP_WS *ws, size_t m, int i) 
{
    ...
    if (m == 1) {
        ...
        if (i == 0) {
            gsl_vector_memcpy(ws->x1[0], ws->b);
        } else {
            gsl_vector_memcpy(ws->x2[0], ws->b);
        }
    } else {
        double f = floor(mu);
        double c = ceil(mu);
        ...
        /* Store the result to current level's x-vector
         * (x1[m-1] if i == 0 and x2[m-2] if i == 1) */
        if (i == 0) {
            if (tx1normsquared <= tx2normsquared) {
                gsl_vector_memcpy(ws->x1[m-1], ws->x1[m-2]);
            } else {
                gsl_vector_memcpy(ws->x1[m-1], ws->x2[m-2]);
            }
        } else {
            if (tx1normsquared <= tx2normsquared) {
                gsl_vector_memcpy(ws->x2[m-1], ws->x1[m-2]);
            } else {
                gsl_vector_memcpy(ws->x2[m-1], ws->x2[m-2]);
            }
        }
    }
}
...
void doubleplane_g(gsl_vector* clp, const gsl_vector* t, const gsl_matrix* B, void* ws)
{
    ...
    doubleplane(B, t, dp_ws, B->size2, 0);
    gsl_vector_memcpy(clp, dp_ws->x1[B->size2-1]);
}
